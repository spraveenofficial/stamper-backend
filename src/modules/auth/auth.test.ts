...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/logout', () => {
    test('should return 204 if refresh token is valid', async () => {
      ...
      await tokenService.saveToken(refreshToken, userOne._id, userOne.role, expires, tokenTypes.REFRESH);
    ...
    test('should return 404 error if refresh token is blacklisted', async () => {
      ...
      await tokenService.saveToken(refreshToken, userOne._id, userOne.role, expires, tokenTypes.REFRESH, true);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      await tokenService.saveToken(resetPasswordToken, userOne._id, userOne.role, expires, tokenTypes.RESET_PASSWORD);
    ...
    test('should return 401 if reset password token is blacklisted', async () => {
      ...
      await tokenService.saveToken(resetPasswordToken, userOne._id, userOne.role, expires, tokenTypes.RESET_PASSWORD, true);
  ...
  describe('POST /v1/auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      await tokenService.saveToken(verifyEmailToken, userOne._id, userOne.role, expires, tokenTypes.VERIFY_EMAIL);
    ...
    test('should return 401 if verify email token is blacklisted', async () => {
      ...
      await tokenService.saveToken(verifyEmailToken, userOne._id, userOne.role, expires, tokenTypes.VERIFY_EMAIL, true);
    ...
    test('should return 401 if verify email token is expired', async () => {
      await insertUsers([userOne]);
      const expires = moment().subtract(1, 'minutes');
      const verifyEmailToken = tokenService.generateToken(userOne._id, expires, tokenTypes.VERIFY_EMAIL,config.jwt.secret, userOne.role);
      await tokenService.saveToken(verifyEmailToken, userOne._id, userOne.role, expires, tokenTypes.VERIFY_EMAIL);
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is generated with an invalid secret', async () => {
    await insertUsers([userOne]);
    const expires = moment().add(config.jwt.accessExpirationMinutes, 'minutes');
    const accessToken = tokenService.generateToken(userOne._id, expires, tokenTypes.ACCESS, 'invalidSecret', userOne.role);
  ...
  test('should call next with unauthorized error if access token is expired', async () => {
    await insertUsers([userOne]);
    const expires = moment().subtract(1, 'minutes');
    const accessToken = tokenService.generateToken(userOne._id, expires, tokenTypes.ACCESS, config.jwt.secret, userOne.role);
    const req = httpMocks.createRequest({ headers: { Authorization: `Bearer ${accessToken}` } });
    const next = jest.fn();
